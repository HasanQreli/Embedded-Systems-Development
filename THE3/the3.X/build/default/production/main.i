...
output_st_t output_st = DIST;

void output_task();


typedef enum {IDLEE, clear, A, B, C, D} led_cmd_t;
led_cmd_t led_cmd = IDLEE;
...
int press_counter[8] = {0, 0, 0, 0, 0, 0, 0, 0};
int rb_threshold = 5;
int release_counter[8] = {0, 0, 0, 0,0, 0, 0, 0};
uint8_t prevB;
...
void init_ports() {
    LATB = 0x00; PORTB = 0x00; TRISB = 0xF0;
    LATD = 0x00; PORTD = 0x00; TRISD = 0x00;
    LATH = 0x00; PORTH = 0x00; TRISH = 0x10;
...
void calc_task() {
    ...
    if(strcmp(cmd, "GOO") == 0 && pkt_bodysize == 7){
        ...
        sscanf(pkt_body+3, "%04x", &alt_period);
        if(alt_period == 0){
            counter_limit = 0;
            counter = 0;
        }
    ...
    else if(started && strcmp(cmd, "END") == 0 && pkt_bodysize == 3){
        disable_rxtx();
    }
